...
function EditProfile({
  ...
  const mut = useMutation({
    ...
    onSuccess: (data, variables) => {
      // Instant UI update: the server already returned the fresh row, so put
      // it straight into the cache instead of waiting on a refetch.
      if (data) qc.setQueryData(["profile"], data);
      onSaved((variables as any).language as Lang);
    },
    onError: (error) => {
      console.error("updateMyProfile failed:", error);
    },
